...
/**
 * Contract: POST /store/membership-discounts/apply
 * Increments metadata counters for a specific rule and appends usage event.
 */
export const POST = async (req: MedusaRequest<Body>, res: MedusaResponse) => {
  ...
  if (!check.allowed) {
    return res.status(409).json({
      error: "Rule usage limit reached",
      rule_id: rule.id,
  ...
  const nextMetadata = svc.incrementRuleCounter(metadata, rule.id, rule.period, at)
